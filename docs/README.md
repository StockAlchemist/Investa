...
* **`$CASH` Symbol:** This is a special reserved symbol used to manage cash balances within accounts. The currency of the cash is determined by the currency assigned to the transaction's account (see **Settings > Account Currencies...**).
  * **Symbol:** Use the single symbol **`$CASH`** for all cash-related transactions (deposits, withdrawals, etc.). The price for `$CASH` is always `1.0` in its account's local currency.
  * **Deposits:** Record a "Deposit" transaction with the `$CASH` symbol, a positive quantity, and a price of 1. The "Total Amount" should be the deposit amount.
  * **Withdrawals:** Record a "Withdrawal" transaction with the `$CASH` symbol, a positive quantity, and a negative "Total Amount".
  * **Internal Cash Movements & TWR:** For accurate Time-Weighted Return (TWR) calculations, it's vital to log all external cash inflows and outflows. Use `Deposit` and `Withdrawal` types for this. Internal movements, like receiving cash from a stock sale, should be recorded as a `Buy` of `$CASH`.
