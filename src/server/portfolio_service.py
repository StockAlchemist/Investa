...
def clear_portfolio_caches():
    ...
    _RAW_CALC_CACHE.clear()
    logging.info(
        "Portfolio Summary, Raw, Market History, and Portfolio History caches cleared (Transaction cache retained)."
    )
...
def trigger_background_precalculation(current_user: User):
    ...
    if current_user.username in _PRECALC_IN_FLIGHT:
        logging.debug(
            f"Precalc already in flight for {current_user.username}, skipping duplicate submission."
        )
        return
    ...
    def run_precalc():
        try:
            logging.info(
                f"Starting background metric pre-calculation for user {current_user.username}"
            )
            from server.dependencies import get_transaction_data

            # retrieve user transaction data manually
            df, manual, user_map, excluded, acc_curr, cash_mode, path, mtime = (
                get_transaction_data(current_user)
            )
            if df.empty:
            ...
            if not conn:
                logging.error(
                    f"Failed to connect to user database for precalculation: {db_path}"
                )
                return
            ...
            try:
                ...
                conn.close()
            logging.info(
                f"Finished background metric pre-calculation for user {current_user.username}"
            )
        except Exception as e:
            logging.error(
                f"Error in background metric pre-calculation: {e}", exc_info=True
            )
        finally:
...
async def warm_summary_caches(max_users: int = 2) -> None:
            ...
            (
                ...
                if os.path.isfile(
                    db_path := os.path.join(
                        users_root, name, config.PORTFOLIO_DB_FILENAME
                    )
                )
            ),
            reverse=True,
    ...
    for _, username in candidates[:max_users]:
        try:
            ...
            await get_asset_change(
                currency=currency, accounts=None, benchmarks=benchmarks, data=data
            )
            logging.info(
...
async def _get_historical_performance_cached(
    ...
    async def compute():
        logging.info(
            f"Calculating historical performance (SWR) for key {cache_key[:3]}..."
        )
...
def compute_account_closure_state(
    ...
    all_selected_closed = bool(closed_in_slice) and len(closed_in_slice) == len(
        include_accounts
    )
...
async def _compute_raw_summary(
    ...
    async def compute():
        ...
        try:
            ...
            logging.error(
                f"Error in calculate_portfolio_summary: {e_calc}", exc_info=True
            )
            raise HTTPException(
                status_code=500, detail=f"Calculation Error: {str(e_calc)}"
            )
...
async def _calculate_portfolio_summary_internal(
    ...
    cache_key = (currency, accounts_key, db_path, db_mtime, time_key)
        ...
        try:
            ...
            if not df_for_twr.empty:
                ...
                (
                    ...
                    _,
                    hist_status,
                ) = await _get_historical_performance_cached(
                    df=df,
                    manual_overrides_dict=manual_overrides,
                    user_symbol_map=user_symbol_map,
                    user_excluded_symbols=user_excluded_symbols,
    ...
    if overall_summary_metrics:
        ...
        if not all_selected_closed:
            ...
            if abs(total_buy_cost) > 1e-9:
                ...
                if days_since_inception > 0:
                    ...
                    if div_factor > 0:
                        annual_div_factor = div_factor ** (
                            365.25 / days_since_inception
                        )
                        overall_summary_metrics["dividend_return_annualized"] = (
                            annual_div_factor - 1
                        ) * 100.0
            # --- END NEW ---
        ...
        logging.info(
            f"Summary calculated. Total Value: {overall_summary_metrics.get('market_value')}, Day Change: {overall_summary_metrics.get('day_change_display')}"
        )
    else:
...
async def _calculate_historical_performance_internal(
    ...
    if not df.empty and "Date" in df.columns:
        ...
        if accounts and "Account" in df.columns:
            ...
            logging.info(
                f"DEBUG_CLAMP: Received accounts={accounts}. Available in DF: {unique_accts}"
            )
            ...
            if mask.any():
                ...
                logging.info(
                    "DEBUG_CLAMP: No transactions found for selected accounts."
                )
        ...
        logging.info(
            f"DEBUG_CLAMP: Accounts={accounts}, Period={period}, RequestStart={start_date}, EarliestDT={earliest_dt}"
        )
    ...
    logging.info(
        f"API History: Mapped benchmarks: {benchmarks}. TickerToName: {ticker_to_name}"
    )

    logging.info(
        f"API History: Fetching period='{period}', interval='{interval}', start={start_date}, end={end_date}, benchmarks={benchmarks}"
    )
    ...
    if daily_df is not None and not daily_df.empty:
        ...
        if not isinstance(daily_df.index, pd.DatetimeIndex):
            logging.info(
                f"DEBUG_INDEX: Index is {type(daily_df.index)}. Columns: {daily_df.columns}"
            )
        ...
        if "Portfolio Accumulated Gain" in daily_df.columns:
            daily_df["Portfolio Accumulated Gain"] = daily_df[
        ...
        if True:  # period != "all":
            try:
                ...
                if not df_before.empty:
                    ...
                    logging.info(
                        f"API: Filtered baseline to 1 point at {baseline_point.index[0]}"
                    )
                else:
                    daily_df = df_display
    ...
    ):  # Modified: Always normalize, even for "all"
        try:
            ...
            if "Portfolio Accumulated Gain" in daily_df.columns:
                ...
                if (
                    start_val != 1.0
                    or daily_df["Portfolio Accumulated Gain"].iloc[0] == 0
                ):
                    daily_df["Portfolio Accumulated Gain"] = (
                        daily_df["Portfolio Accumulated Gain"] / start_val
                    )
            ...
            if benchmarks:
                for b_ticker in benchmarks:
                    ...
                    if bm_price_col in daily_df.columns:
                        num_tuesday = (
                            daily_df.loc[
                                daily_df.index.date == date(2026, 1, 20), bm_price_col
                            ]
                            .notna()
                            .sum()
                        )
                        logging.info(
                            f"Benchmark {b_ticker}: Aligned. Tuesday points: {num_tuesday}"
                        )
        ...
        if fx_pair in historical_fx_yf:
            ...
            rate_col = (
                ...
                if "price" in fx_df.columns
                    ...
                    else ("Adj Close" if "Adj Close" in fx_df.columns else None)
                )
            )
            ...
            if not fx_df.empty and rate_col:
                ...
                fx_rate_series = fx_df_proc[rate_col].reindex(
    ...
    try:
        ...
        tradable_days = set(
            cal.schedule(start_date=start_date, end_date=end_date).index.date
        )
    except Exception as e_cal:
    ...
    for i, (dt, row) in enumerate(daily_df.iterrows()):
        ...
        if not is_baseline and interval in [
            "1m",
            "2m",
            "5m",
            "15m",
        ...
        item = {
            ...
            "twr": (twr - 1) * 100
            if pd.notnull(twr)
            else 0.0,  # Convert to percentage change
            "drawdown": dd
            if pd.notnull(dd)
            else 0.0,  # Already percentage from portfolio_logic
    ...
    logging.info(
        f"API History: Input {total_input} rows. Filtered: Pre={pre_market_count}, Post={post_market_count}. Output {len(result)} rows."
    )
