...
def main():
    ...
    if conn:
        try:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT DISTINCT symbol FROM screener_cache WHERE ai_summary IS NOT NULL AND ai_summary != ''"
            )
    ...
    logging.info(
        f"Identified {len(missing_symbols)} missing symbols to sync: {missing_symbols}"
    )
    ...
    for i, symbol in enumerate(missing_symbols):
        ...
        try:
            ...
            if not fund_data:
                logging.warning(
                    f"Could not fetch live fund data for {symbol}, using minimal placeholders."
                )
