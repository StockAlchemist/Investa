...
def get_etf_holdings(product_id: str, ticker: str, filename: str) -> List[str]:
    ...
    try:
        ...
        if "<html" in content.lower():
            raise ValueError(
                f"Received HTML instead of CSV (likely bot protection) for {ticker}"
            )
...
def get_sp500_tickers() -> List[str]:
    ...
    cache_path = os.path.join(
        config.get_app_data_dir(), config.CACHE_DIR, SP500_CACHE_FILE
    )
...
def get_russell2000_tickers() -> List[str]:
    ...
    """
    cache_path = os.path.join(
        config.get_app_data_dir(), config.CACHE_DIR, "russell2000_tickers_cache.json"
    )
    ...
    try:
        ...
        if "Ticker" not in df.columns:
            logging.error(
                f"Russell 2000 GitHub CSV missing ticker column. Found: {df.columns}"
            )
            return []
...
def get_sp400_tickers() -> List[str]:
    ...
    """
    cache_path = os.path.join(
        config.get_app_data_dir(), config.CACHE_DIR, "sp400_tickers_cache.json"
    )
    ...
    try:
        ...
        if col not in df.columns:
            logging.error(
                f"S&P 400 Wikipedia table missing ticker column. Found: {df.columns}"
            )
            return []
...
def screen_stocks(
    ...
    if fast_mode:
        try:
            ...
            if cached_results:
                logging.info(
                    f"Screener: FAST MODE requested. Returning {len(cached_results)} cached items immediately."
                )
                cached_results.sort(
                    key=lambda x: (
        ...
        if universe_type == "holdings":
            ...
            try:
                wl_data = get_watchlist(
                    conn, int(universe_id)
                )  # This returns list of Dict with 'symbol'
                if wl_data:
            ...
            if raw_cached:
                ...
                if all_timestamps:
                    ...
                    if latest_update_ts.date() == now_ts.date():
                        ...
                        if age_seconds < 3600:
                            logging.info(
                                f"Screener: Pure Fast-path load for '{universe_tag}' ({len(cached_results)} items, {age_seconds:.0f}s old)"
                            )
                            use_pure_fast_path = True
                        else:
                            logging.info(
                                f"Screener: Stale Fast-path found {len(cached_results)} items. Age: {age_seconds:.0f}s. Refreshing."
                            )
        except Exception as e:
    ...
    if missing_symbols:
        ...
        logging.info(
            f"Screener: Checking cache validity for {len(missing_symbols)} missing symbols before batch fetch..."
        )
        ...
        for sym in missing_symbols:
            ...
            if cached:
                ...
                if (
                    cached.get("last_fiscal_year_end") == live_fy_end
                    and cached.get("most_recent_quarter") == live_quarter
                    and _has_settled_valuation(cached)
                ):
                    can_use_cache = True
        ...
        if needed_statements_syms:
            logging.info(
                f"Screener: Batch fetching financials for {len(needed_statements_syms)} symbols..."
            )
            # Use the new batch method
            batch_statements = mdp.get_financial_statements_batch(
                needed_statements_syms
            )
        else:
            logging.info(
                "Screener: All missing symbols have valid individual cache. Skipping batch fetch."
            )
...
def process_screener_results(
    ...
    if os.path.exists(ai_cache_dir):
        try:
            ...
            existing_reviews = {
                f.split("_")[0].upper() for f in files if f.endswith("_analysis.json")
            }
        except Exception:
            pass
        ...
        try:
            ...
            if cached:
                ...
                if (
                    live_fy_end
                    and live_quarter
                    and cached.get("last_fiscal_year_end") == live_fy_end
                    and cached.get("most_recent_quarter") == live_quarter
                    and _has_settled_valuation(cached)
                ):
                    can_use_cache = True
            ...
            if can_use_cache:
                ...
                ai_cache_path = os.path.join(
                    ai_cache_dir, f"{sym.upper()}_analysis.json"
                )
                use_ai_from_disk = False

                # Default to cache values
                has_ai = (
                    cached.get("has_ai_review")
                    if "has_ai_review" in cached
                    else (cached.get("ai_summary") is not None)
                )
                ...
                if os.path.exists(ai_cache_path):
                    try:
                        ...
                        if cache_ts_str:
                            try:
                                cache_ts = datetime.fromisoformat(
                                    cache_ts_str
                                ).timestamp()
                            except ValueError:
                                pass
                    ...
                    try:
                        with open(ai_cache_path, "r") as f:
                            ...
                            if vals:
                                ...
                                logging.info(
                                    f"Screener: Detected new AI review for {sym}. Reloading from disk."
                                )
                ...
                if has_ai:
                    try:
                        with open(ai_cache_path, "r") as f:
                            ...
                            vals = [
                                v
                                for v in [ai_moat, ai_fin, ai_pred, ai_growth]
                                if isinstance(v, (int, float))
                            ]
                            if vals:
                                ai_score = sum(vals) / len(vals)
    ...
    logging.info(
        f"Screener: Processing results for {len(symbols)} symbols in parallel..."
    )
...
def run_narrative_search(prompt: str) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        gen_config = {"temperature": 0.1, "maxOutputTokens": 2048, "topP": 0.95}
            ...
            try:
                ...
                if "candidates" in data and data["candidates"]:
                    ...
                    if candidate.get("finishReason") == "SAFETY":
                        logging.warning(
                            f"Screener: Model '{model}' triggered safety filter."
                        )
                        continue
        ...
        if not sql:
            logging.error(
                f"Screener: No SQL generated. Last response keys: {data.keys() if 'data' in locals() else 'None'}"
            )
            return []
        ...
        if "```" in sql:
            ...
            match = re.search(
                r"```(?:sql)?\s*(.*?)\s*```", sql, re.DOTALL | re.IGNORECASE
            )
            if match:
                sql = match.group(1).strip()
            else:
                sql = (
                    re.sub(r"```(?:sql)?", "", sql, flags=re.IGNORECASE)
                    .replace("```", "")
                    .strip()
                )
        ...
        if (
            not sql_lower.startswith("select")
            or "screener_cache" not in sql_lower
            or any(x in sql_lower for x in forbidden)
        ):
        ...
        for row in rows:
            ...
            vals = [
                ...
                for v in [
                    item.get("ai_moat"),
                    item.get("ai_financial_strength"),
                    item.get("ai_predictability"),
                    item.get("ai_growth"),
                ]
                if isinstance(v, (int, float))
            ]
