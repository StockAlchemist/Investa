...
# Ensure src is in path so internal imports like 'import config' work
...
def mock_calculate_mtd_adb(current_cash, mtd_transactions, today_date):
    ...
    if mtd_transactions.empty:
        ...
        changes_by_date = (
            mtd_transactions.groupby("Date")["Total Amount"].sum().to_dict()
        )
        # Convert keys to date objects if needed
        changes_by_date = {
            k.date() if hasattr(k, "date") else k: v for k, v in changes_by_date.items()
        }
...
def test_config_manager_settings(tmp_path):
    ...
    assert (
        "account_interest_rates" not in cm.manual_overrides
        or cm.manual_overrides["account_interest_rates"] == {}
    )
...
def test_calculate_mtd_adb():
    ...
    df = pd.DataFrame([{"Date": pd.Timestamp("2025-01-05"), "Total Amount": 5000.0}])
    ...
    df2 = pd.DataFrame([{"Date": pd.Timestamp("2025-01-09"), "Total Amount": -2000.0}])
