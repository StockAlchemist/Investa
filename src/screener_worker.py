            ...
            try:
                ...
                for uni in universes:
                    ...
                    results = screen_stocks(
                        universe_type=uni, db_conn=conn, fast_mode=False
                    )

                    # specific verification
                    valid_count = sum(
                        1 for r in results if r.get("intrinsic_value") is not None
                    )
                    uni_duration = time.time() - uni_start
                    logging.info(
                        f"    - {uni}: {valid_count}/{len(results)} valid. ({uni_duration:.1f}s)"
                    )
...
def main():
    ...
    logging.info(
        f"Target Update Interval: Every {UPDATE_INTERVAL_SECONDS / 3600:.1f} hours."
    )
