...
def _calculate_daily_net_cash_flow_vectorized(
    ...
    if not df_cash.empty:
        ...
        total_amount_col = (
            df_cash["Total Amount"]
            if "Total Amount" in df_cash.columns
            else pd.Series(0.0, index=df_cash.index)
        )
        total_amount = (
            pd.to_numeric(total_amount_col, errors="coerce").fillna(0.0).abs()
        )
    ...
    if included_accounts:
        ...
        if note_col:
            cleanup_mask = (
                ...
                .fillna("")
                .str.lower()
                .apply(lambda x: any(kw in x for kw in cleanup_keywords))
            )

        # Transfers and Cleanup/Adjustments are both treated as boundary-crossing flows
        trans_mask = (
            df_period["Type"].str.lower().str.strip() == "transfer"
        ) | cleanup_mask
        ...
        if not df_trans.empty:
            ...
            is_cleanup = (
                cleanup_mask.reindex(df_trans.index).fillna(False) & ~is_real_transfer
            )
            if is_cleanup.any():
            ...
            if not relevant_trans.empty:
                ...
                qty = (
                    pd.to_numeric(relevant_trans["Quantity"], errors="coerce")
                    .fillna(0.0)
                    .abs()
                )
                price = pd.to_numeric(
                    relevant_trans["Price/Share"], errors="coerce"
                ).fillna(0.0)
                # Ensure $CASH always has price 1.0
                is_cash_mask = (
                    relevant_trans["Symbol"].str.upper() == CASH_SYMBOL_CSV.upper()
                )
                ...
                if missing_price_mask.any():
                    ...
                    for sym in unique_syms:
                        ...
                        sym_mask = (
                            relevant_trans["Symbol"] == sym
                        ) & missing_price_mask
                        price_found = 0.0

                        # Fallback A: Yahoo Finance Lookup
                        yf_sym = (
                            internal_to_yf_map.get(sym) if internal_to_yf_map else None
                        )
                        if (
                            yf_sym
                            and historical_prices_yf_unadjusted
                            and yf_sym in historical_prices_yf_unadjusted
                        ):
                            try:
                                price_series = historical_prices_yf_unadjusted[yf_sym][
                                    "price"
                                ]
                                if not price_series.empty:
                                    ...
                                    if not isinstance(
                                        price_series.index, pd.DatetimeIndex
                                    ):
                                    ...
                                    dates_needed = pd.to_datetime(
                                        relevant_trans.loc[sym_mask, "Date"]
                                    )
                                    # Use reindex with nearest or ffill
                                    relevant_prices = price_series.reindex(
                                        dates_needed, method="ffill"
                                    )
                                    price_found = relevant_prices.iloc[
                                        0
                                    ]  # Take first for simplicity in this loop
                            except Exception as e:
                                logging.warning(
                                    f"Transfer price fallback failed for {sym}: {e}"
                                )
                        ...
                        if price_found > 1e-9:
                            ...
                            try:
                                with open(
                                    "/Users/kmatan/.gemini/antigravity/brain/19ed956f-9dc7-40d0-85b1-1aa49c84c1d9/scratch/missing_transfer_prices.txt",
                                    "a",
                                ) as f:
                ...
                flows_list.append(
                    relevant_trans[["Date", "_flow_local", "Local Currency"]]
                )
    ...
    flows_grouped = (
        all_flows.groupby(["Date", "Local Currency"])["_flow_local"].sum().reset_index()
    )
    ...
    for curr in unique_currs:
        ...
        if curr == target_currency:
            ...
            daily_rate_map = pd.Series(
                cross_rate_s.values, index=cross_rate_s.index.date
            ).to_dict()
    ...
    if final_flows:
            ...
            if isinstance(daily_net_flow_add.index, pd.DatetimeIndex):
                ...
                if target_tz is not None:
                    ...
                    if source_tz is None:
                        ...
                        daily_net_flow_add.index = daily_net_flow_add.index.tz_localize(
                            target_tz
                        )
                    else:
                        # Source is Aware -> Convert to Target
                        daily_net_flow_add.index = daily_net_flow_add.index.tz_convert(
                            target_tz
                        )
                else:
                    # Target is Naive
        ...
        if pre_range_mask.any():
            ...
            logging.info(
                f"STABILIZATION: Found {pre_range_mask.sum()} transactions before start date. Adding Initial Flow of {initial_flow_total:.2f} to {start_ts}"
            )
...
def _calculate_daily_net_cash_flow(
        ...
        for _, row in external_flow_tx.iterrows():
            ...
            if is_cash:
                            ...
                            if target_date in prices_df.index:
                                for col_name in ["Close", "price", "Adj Close"]:
                                    if col_name in prices_df.columns:
                                        price_local = prices_df.loc[
                                            target_date, col_name
                                        ]
                                        break
                                else:
                ...
                if not pd.isna(price_local) and price_local > 0:
                    ...
                    logging.warning(
                        f"Flow: Could not determine price for asset flow {symbol} on {target_date}. Skipping."
                    )
                    continue
    ...
    if included_accounts:
        ...
        transfer_tx = daily_tx[
            daily_tx["Type"].str.lower().str.strip() == "transfer"
        ].copy()
        ...
        for _, row in transfer_tx.iterrows():
            ...
            is_to_included = (
                to_account_norm in included_set if to_account_norm else False
            )
