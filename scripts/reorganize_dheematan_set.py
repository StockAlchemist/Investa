...
def norm_symbol(raw):
    ...
    r = raw.strip()
    aliases = {
        "es-fixed_income": "ES-FIXED_INCOME",
        "es-tresury": "ES-TRESURY",
        "es-jumbo25": "ES-JUMBO25",
        "es-set50": "ES-SET50",
        "es-gqg": "ES-GQG",
    }
...
def stock_target(row, pairs_larger):
    ...
    """
    sym, date, typ, qty, amt = (
        row["Symbol"],
        row["Date"],
        row["Type"],
        row["Quantity"] or 0.0,
        abs(row["Total Amount"] or 0.0),
    )
    ...
    if sym == "TCAP:BKK":
        if typ == "Buy":
            ...
            return (
                SCBS,
                NOTE_KIT,
            )  # 2014-07-08, 2014-09-29, 2018-06-06 (split row handled separately)
        if typ == "Sell":
            if date == "2015-02-06":
                return KIM_ENG, None
            if date == "2024-02-06":
...
def cash_target(row, trade_assign, div_net_assign):
    ...
    if m:
        ...
        if m.group("div"):
            ...
            cands = {
                a for (s, d, _), a in div_net_assign.items() if s == sym and d == date
            }
            if len(cands) == 1:
                return cands.pop()
        ...
        cands = {
            a
            for (s, d, _, k), a in trade_assign.items()
...
def main():
    ...
    rows = cur.execute(
        "SELECT * FROM transactions WHERE Account='SET' ORDER BY Date, id"
    ).fetchall()
    if not rows:
    ...
    for r in rows:
        if r["Symbol"] in ("ZEN:BKK", "PTT:BKK", "TCAP:BKK", "AMARIN:BKK") and r[
            "Type"
        ] in ("Dividend", "Tax"):
            by_sdt[(r["Symbol"], r["Date"], r["Type"])].append(
                abs(r["Total Amount"] or 0.0)
            )
    ...
    print(
        "  + 1 TCAP buy row and its cash deposit split 3,000 (SCBS) / 2,500 (Kim Eng)"
    )
    print(
        "  + corporate-action rows: MBK split, AMARIN stock dividend (970 KE / 750 SCBS),"
    )
    ...
    ins_sql = (
        f"INSERT INTO transactions ({','.join(chr(34) + c + chr(34) for c in ins_cols)}) "
    ...
    with con:
        ...
        for r in rows:
            ...
            cur.execute(
                "UPDATE transactions SET Account=?, Note=? WHERE id=?",
                (acct, note, r["id"]),
            )
        ...
        cur.execute(
            ...
            (
                SCBS,
                TCAP_SPLIT_BUY["kit_qty"],
                TCAP_SPLIT_BUY["kit_qty"] * 36.0,
                round(comm * f_kit, 2),
                append_note(
                    t["Note"],
                    NOTE_KIT + " | split from 5,500-sh row (2,500 at Kim Eng)",
                ),
                t["id"],
            ),
        )
        cur.execute(
            ...
            (
                ...
                "Buy",
                "TCAP:BKK",
                TCAP_SPLIT_BUY["ke_qty"],
                36.0,
                TCAP_SPLIT_BUY["ke_qty"] * 36.0,
                round(comm * (1 - f_kit), 2),
                KIM_ENG,
                None,
                "split from 5,500-sh row (3,000 at SCBS)",
                "THB",
                None,
                user_id,
            ),
        ...
        cur.execute(
            'UPDATE transactions SET Account=?, Quantity=?, "Total Amount"=? WHERE id=?',
            (
                SCBS,
                TCAP_SPLIT_BUY["kit_qty"] * 36.0,
                TCAP_SPLIT_BUY["kit_qty"] * 36.0,
        ...
        cur.execute(
            ...
            (
                ...
                KIM_ENG,
                None,
                "AMARIN 10% stock dividend (9,700 sh held)",
                "THB",
                None,
                user_id,
            ),
        ...
        cur.execute(
            "UPDATE transactions SET Symbol='BECL:BKK', Note=? "
        ...
        cur.execute(
            "UPDATE transactions SET Symbol='BEM:BKK', "
            "Note=COALESCE(Note,'') || ' | relabeled from BML' "
            "WHERE Symbol='BML:BKK'"
        )
        cur.execute(
            "UPDATE transactions SET Note=REPLACE(Note,'BML:BKK','BEM:BKK') "
    ...
    if args.config:
        ...
        for gname, members in groups.items():
            if "SET" in members:
                ...
                members[idx : idx + 1] = (
                    [EASTSPRING, SCBAM] if gname == "Current Holdings" else new_accts
                )
    ...
    print("\nVerification:")
    left = cur.execute(
        "SELECT COUNT(*) FROM transactions WHERE Account='SET'"
    ).fetchone()[0]
    ...
    print(
        ...
        dict(
            cur.execute(
                ...
                "GROUP BY Account",
                (EASTSPRING, SCBAM, UOBAM, KIM_ENG, SCBS),
            ).fetchall()
        ),
    )
    bad = []
    ...
    recv = dict(
        cur.execute(
            ...
            "WHERE Type='Transfer' AND \"To Account\"=? GROUP BY Symbol",
            (SCBS,),
        ).fetchall()
    )
    ...
    print(
        "  open positions:",
        [(a, s, round(q or 0, 2)) for a, s, q in net if s in open_expected],
    )
    con.close()
    print("\nDone.")
