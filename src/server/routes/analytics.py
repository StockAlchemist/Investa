...
async def _generate_dividend_events(
        ...
        try:
            ...
            if indicated_rate > 0:
                ...
                if local_events:
                    try:
                        anchor = datetime.strptime(
                            local_events[-1]["dividend_date"], "%Y-%m-%d"
                        ).date()
                ...
                if anchor:
                    ...
                    while curr < today:
                        curr = (
                            pd.Timestamp(curr) + pd.DateOffset(months=freq_months)
                        ).date()
                    ...
                    while curr <= end_date:
                        ...
                        for ce in local_events:
                            if ce["status"] == "confirmed":
                                ce_date = datetime.strptime(
                                    ce["dividend_date"], "%Y-%m-%d"
                                ).date()
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
        future_to_sym = {
            executor.submit(fetch_symbol_data, sym): sym for sym in holdings.keys()
        }
    ...
    try:
        ...
        interest_rates = config_manager.manual_overrides.get(
        ...
        try:
            ...
            if cash_events:
                logging.info(
                    f"Generated {len(cash_events)} cash interest events for {current_user.username}"
                )
    ...
    try:
        ...
        for event in events:
            ...
            try:
                ...
                amt = event[
                    "amount"
                ]  # This is usually LOCAL amount (except for Cash Interest)
...
async def get_risk_metrics(
    ...
    try:
        ...
        benchmark_values = (
            daily_df["^GSPC Price"] if "^GSPC Price" in daily_df.columns else None
        )
        ...
        try:
            if not daily_df.empty:
                ...
                if not prev_year_data.empty:
                    prev_year_twr = prev_year_data["Portfolio Accumulated Gain"].iloc[
                        -1
                    ]
        ...
        if metrics.get("Beta") is None:
            logging.warning(
                f"Risk Metrics: Beta is None. daily_df columns: {daily_df.columns.tolist()}, port_len={len(portfolio_values)}, bench_len={len(benchmark_values) if benchmark_values is not None else 0}"
            )
    ...
    try:
        ...
        for row in rows:
            ...
            for prev_item in stock_data:
                ...
                name_match = (
                    name and prev_item.get("name") and name == prev_item.get("name")
                )
        ...
        for stock in stock_data:
            stock["contribution"] = (
                (stock["gain"] / total_cost) if total_cost != 0 else 0.0
            )
        ...
        return clean_nans(
            {
                ...
                "stocks": stock_data
                if show_all
                else stock_data[:10],  # Top 10 contributors/detractors by default
...
def calculate_mtd_average_daily_balance(
    current_cash: float, mtd_transactions: pd.DataFrame, today_date: date
) -> float:
    """
    ...
    if not mtd_transactions.empty:
        ...
        changes_by_date = (
            mtd_transactions.groupby(mtd_transactions["Date"].dt.date)["Total Amount"]
            .sum()
            .to_dict()
        )
