...
def generate_stock_review(
    ...
    """
    logging.info(
        f"AI Analysis: Generating review for {symbol} (force_refresh={force_refresh})"
    )
            ...
            with open(cache_path, "r") as f:
                ...
                if not force_refresh:
                    ...
                    if fund_data and "lastFiscalYearEnd" in fund_data:
                        ...
                        cached_fy = cached_data.get("metadata", {}).get(
                            "lastFiscalYearEnd"
                        )
                        cached_q = cached_data.get("metadata", {}).get(
                            "mostRecentQuarter"
                        )
                        ...
                        if (
                            live_fy
                            and cached_fy
                            and (live_fy != cached_fy or live_q != cached_q)
                        ):
                            logging.info(
                                f"AI Cache: New financial report detected (FY: {live_fy}, Q: {live_q}). Invalidating cache for {symbol}."
                            )
                            is_cache_valid = False

                    # Also check TTL (legacy fallback if no metadata found)
                    if is_cache_valid and (
                        time.time() - cache_time > config.AI_REVIEW_CACHE_TTL
                    ):
                        logging.info(
                            f"AI Cache: TTL expired for {symbol}. Attempting refresh."
                        )
                        is_cache_valid = False
                    ...
                    if stale_result and "error" not in stale_result:
                        try:
                            ...
                            update_ai_review_in_cache(
                                symbol, stale_result, info=fund_data
                            )
                        except Exception as e_sync:
                            logging.warning(
                                f"AI Analysis: Failed to sync disk-cache to DB for {symbol}: {e_sync}"
                            )
    ...
    if not force_refresh:
        try:
            ...
            if symbol in db_results:
                ...
                if row.get("ai_summary") and len(row["ai_summary"]) > 20:
                    ...
                    if any(v and v != "N/A" for v in analysis.values()):
                        logging.info(
                            f"AI Cache: Found valid review in DB for {symbol}. Returning."
                        )
                        return db_result
                    logging.info(
            ...
            logging.warning(
                f"AI Cache: Failed to read from DB for {symbol}: {e_db_cache}"
            )
...
"""
    ...
    for model in FALLBACK_MODELS:
        ...
        for current_search_setting in search_options:
            logging.info(
                f"AI Analysis: Attempting review for {symbol} using model '{model}' (Search: {current_search_setting})..."
            )
            ...
            try:
                ...
                if response.status_code == 429 or 500 <= response.status_code < 600:
                    logging.warning(
                        f"AI Analysis: Model '{model}' (Search: {current_search_setting}) failed (status {response.status_code}). Response: {response.text[:200]}..."
                    )
                ...
                try:
                    ...
                    logging.warning(
                        f"JSON Parse Error for {model}: {e}. Content preview: {content_text[:100]}..."
                    )
                ...
                try:
                    ...
                    try:
                        ...
                        logging.warning(
                            f"Failed to sync AI review to DB cache for {symbol}: {e_db}"
                        )
                ...
                logging.warning(
                    f"AI Analysis: Model '{model}' (Search: {current_search_setting}) failed completely. Error: {error_msg}"
                )
                continue  # Try next option or next model
