...
and its walk-forward backtest (projections.backtest_projection).
...
def _dated_wealth(years=20, mu=0.10, sigma=0.15, seed=11, freq="D"):
    """A synthetic *calendar*-daily wealth index, like the portfolio TWR series."""
    idx = pd.date_range("2000-01-01", periods=int(years * 365), freq=freq)
    ppy = 365.0
    rng = np.random.default_rng(seed)
    daily = rng.normal(mu / ppy, sigma / math.sqrt(ppy), len(idx))
    return pd.Series(100.0 * np.exp(np.cumsum(daily)), index=idx)


def test_backtest_needs_history_for_fit_plus_horizon():
    # Each horizon needs the 5y fitting window plus the horizon itself on top,
    # and enough anchors after that to say anything.
    res = backtest_projection(_dated_wealth(years=4))
    assert res["available"] is False
    assert res["reason"] == "insufficient_history"

    res = backtest_projection(_dated_wealth(years=8))
    assert res["available"] is True
    assert [h["years"] for h in res["horizons"]] == [1]

    res = backtest_projection(_dated_wealth(years=12))
    assert [h["years"] for h in res["horizons"]] == [1, 3, 5]


def test_backtest_reports_calibration_and_replay():
    wealth = _dated_wealth(years=25, seed=12)
    res = backtest_projection(wealth, value_series=wealth * 1000.0)
    assert res["available"] is True
    assert [h["years"] for h in res["horizons"]] == [1, 3, 5, 10]
    for h in res["horizons"]:
        assert h["samples"] > 0
        assert 0.0 <= h["in_band_pct"] <= 100.0
        assert 0.0 <= h["mean_u"] <= 1.0
        assert h["verdict"] in ("calibrated", "narrow", "wide")

    replay = res["replay"]
    # Replays the longest verifiable horizon, ending at the last observation.
    assert replay["years"] == 10
    assert replay["indexed"] is False
    assert replay["points"][0]["actual"] == replay["start_value"]
    assert replay["points"][-1]["date"] == res["history_end"]
    for pt in replay["points"][1:]:
        assert pt["p10"] < pt["p25"] < pt["median"] < pt["p75"] < pt["p90"]
    assert replay["outcome"] in ("inside", "below", "above")


def test_replay_is_indexed_without_a_value_series():
    res = backtest_projection(_dated_wealth(years=20, seed=13))
    assert res["replay"]["indexed"] is True
    assert res["replay"]["start_value"] == 100.0


def test_backtest_uses_no_future_data():
    # Truncating the series must not change the errors measured on the part that
    # survives: each anchor is fit only on data that existed at that date.
    wealth = _dated_wealth(years=20, seed=14)
    full = walk_forward_errors(wealth, horizons=[1])
    early = walk_forward_errors(wealth.iloc[: -365 * 5], horizons=[1])
    merged = full.merge(early, on=["years", "anchor"], suffixes=("_f", "_e"))
    assert len(merged) == len(early)
    assert merged["z_f"].to_numpy() == pytest.approx(merged["z_e"].to_numpy())


def test_standardized_errors_are_calibrated_on_lognormal_data():
    # Data generated by the model's own process: the 1y bands should be roughly
    # right (this is the sanity check that z is computed the right way round).
    errors = walk_forward_errors(_dated_wealth(years=30, seed=15), horizons=[1])
    stats = summarize_errors(errors, [1])[0]
    assert 0.6 < stats["std_z"] < 1.5
    assert 60.0 < stats["in_band_pct"] <= 100.0


def test_summarize_errors_flags_narrow_bands():
    # Errors twice as spread as the model claimed -> "narrow" (overconfident).
    rng = np.random.default_rng(3)
    errors = pd.DataFrame(
        {
            "years": 1,
            "anchor": pd.date_range("2010-01-01", periods=200, freq="ME"),
    ...
    stats = summarize_errors(errors, [1])[0]
    assert stats["verdict"] == "narrow"
    assert stats["std_z"] > 1.5
    assert stats["in_band_pct"] < 80.0
