...
try:
    ...
    # --- End Import ---
    from ui_components.fundamental_data_dialog import FundamentalDataDialog
    from ui_components.log_viewer_dialog import LogViewerDialog
    from ui_components.account_currency_dialog import AccountCurrencyDialog
    from ui_components.symbol_chart_dialog import SymbolChartDialog
    from ui_components.manual_price_dialog import ManualPriceDialog
    from ui_components.add_transaction_dialog import AddTransactionDialog
    from ui_components.manage_transactions_dialog_db import ManageTransactionsDialogDB
    import inspect
...
from gui_utils import get_column_definitions
...
class FundamentalDataWorker(QRunnable): # This class should remain in main_gui.py
...
"""
    ...
    def __init__(self):
        ...
        # Other Tab Widgets
        self._init_transactions_log_tab_widgets()
        self._init_asset_allocation_tab_widgets()
        self._init_dividend_history_tab_widgets()
        self._init_capital_gains_tab_widgets()

        self._create_status_bar() # Moved from _init_ui_widgets
        logging.debug("--- PortfolioApp __init__: After _init_ui_widgets (now granular calls) ---")
        self._init_toolbar()
        self._connect_signals()
        self._apply_initial_styles_and_updates()

        # --- Initial Theme Application ---
        self._config_already_saved_for_theme = True # Prevent save_config during init
        self.apply_theme(self.current_theme)
        if self.current_theme == "light" and hasattr(self, "light_theme_action"):
            self.light_theme_action.setChecked(True)
        elif self.current_theme == "dark" and hasattr(self, "dark_theme_action"):
        ...
        if self.db_conn and self.config.get("load_on_startup", True):
            csv_for_potential_migration = self.config.get("transactions_file_csv_fallback", DEFAULT_CSV)
            logging.info(
...
"""
    ...
    def _init_ui_widgets(self):
        """Orchestrates the initialization of all UI widgets. Now calls granular helpers."""
        # This method is now a placeholder or can be removed if __init__ calls all helpers directly.
        # For this refactoring, we'll assume __init__ takes over orchestration.
        pass

    def _init_dividend_history_tab_widgets(self):
        """Initializes widgets for the Dividend History tab."""
        logging.debug("--- _init_dividend_history_tab_widgets: START ---")
        self.dividend_history_tab = QWidget()
        dividend_history_layout = QVBoxLayout(self.dividend_history_tab)
        dividend_history_layout.setContentsMargins(10, 10, 10, 10)
        ...
        self.dividend_period_combo.setMinimumWidth(100)
        ...
        saved_div_periods_to_show = self.config.get("dividend_periods_to_show", 10)
        ...
        est_annual_income_layout.setContentsMargins(0, 5, 0, 5)
        ...
        self.dividend_bar_fig = Figure(figsize=(7, 3), dpi=CHART_DPI)
        if saved_div_agg_period in ["Annual", "Quarterly", "Monthly"]:
            self.dividend_period_combo.setCurrentText(saved_div_agg_period)
        else:
            self.dividend_period_combo.setCurrentText("Annual")
        self.dividend_periods_spinbox.setValue(saved_div_periods_to_show)
        ...
        dividend_history_layout.addWidget(self.dividend_bar_canvas, 1)

        self.dividend_summary_table_view = QTableView()
        self.dividend_summary_table_view.setObjectName("DividendSummaryTable")
        self.dividend_summary_table_model = PandasModel(parent=self, log_mode=True)
        ...
        self.dividend_summary_table_view.setSortingEnabled(True)
        self.dividend_summary_table_view.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
        ...
        self.dividend_table_model = PandasModel(parent=self, log_mode=True)
        ...
        self.dividend_table_view.horizontalHeader().setSectionResizeMode(QHeaderView.Interactive)
        ...
        summary_group_layout.addWidget(self.dividend_summary_table_view, 1)
        tables_horizontal_layout.addWidget(summary_group, 1)
        transaction_group = QGroupBox("Dividend Transaction History")
        self.dividend_transaction_group = transaction_group
        transaction_group_layout = QVBoxLayout(self.dividend_transaction_group)
        transaction_group_layout.addWidget(self.dividend_table_view, 1)
        tables_horizontal_layout.addWidget(transaction_group, 2)
        dividend_history_layout.addLayout(tables_horizontal_layout, 2)

        self.dividend_history_tab.setVisible(True)
        self.main_tab_widget.addTab(self.dividend_history_tab, "Dividend")
        logging.debug("--- _init_dividend_history_tab_widgets: END ---")

    def _init_capital_gains_tab_widgets(self):
        """Initializes widgets for the Capital Gains tab."""
        logging.debug("--- _init_capital_gains_tab_widgets: START ---")
        # self.capital_gains_tab is already created in _init_ui_structure
        ...
        self.cg_periods_spinbox.setMaximum(50)
        ...
        self.cg_bar_fig = Figure(figsize=(7, 3), dpi=CHART_DPI)
        ...
        cg_main_layout.addWidget(self.cg_bar_canvas, 1)

        tables_horizontal_layout_cg = QHBoxLayout()
        summary_table_group_cg = QGroupBox("Summary of Plotted Gains")
        summary_table_group_layout_cg = QVBoxLayout(summary_table_group_cg)
        ...
        # ... (other cg_summary_table_view properties)
        ...
        self.cg_summary_table_view.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
        self.cg_summary_table_view.verticalHeader().setVisible(False)
        self.cg_summary_table_view.setMinimumHeight(80)
        summary_table_group_layout_cg.addWidget(self.cg_summary_table_view)
        tables_horizontal_layout_cg.addWidget(summary_table_group_cg, 1)

        history_table_group_cg = QGroupBox("Detailed Capital Gains History")
        history_table_group_layout_cg = QVBoxLayout(history_table_group_cg)
        ...
        # ... (other cg_table_view properties)
        ...
        history_table_group_layout_cg.addWidget(self.cg_table_view)
        tables_horizontal_layout_cg.addWidget(history_table_group_cg, 2)
        cg_main_layout.addLayout(tables_horizontal_layout_cg, 2)
        logging.debug("--- _init_capital_gains_tab_widgets: END ---")
    ...
    def _init_transactions_log_tab_widgets(self): # Corrected method name based on intent
        """Initializes widgets for the Transactions Log tab."""
        logging.debug("--- _init_transactions_log_tab_widgets: START ---")
        ...
        self.stock_transactions_table_model = PandasModel(parent=self, log_mode=True)
        ...
        self.stock_transactions_table_view.horizontalHeader().setSectionResizeMode(QHeaderView.Interactive)
        ...
        self.cash_transactions_table_model = PandasModel(parent=self, log_mode=True)
        ...
        self.cash_transactions_table_view.horizontalHeader().setSectionResizeMode(QHeaderView.Interactive)
        ...
        logging.debug("--- _init_transactions_log_tab_widgets: END ---")

    def _init_asset_allocation_tab_widgets(self):
        """Initializes widgets for the Asset Allocation tab."""
        logging.debug("--- _init_asset_allocation_tab_widgets: START ---")
        ...
        row1_charts_layout = QHBoxLayout(row1_charts_container)
        ...
        row1_charts_layout.addWidget(asset_type_group, 1)

        sector_allocation_group = QGroupBox("Allocation by Sector")
        sector_allocation_layout = QVBoxLayout(sector_allocation_group)
        self.sector_pie_fig = Figure(figsize=PIE_CHART_FIG_SIZE, dpi=CHART_DPI)
        self.sector_pie_ax = self.sector_pie_fig.add_subplot(111)
        self.sector_pie_canvas = FigureCanvas(self.sector_pie_fig)
        self.sector_pie_canvas.setObjectName("SectorPieCanvas")
        sector_allocation_layout.addWidget(self.sector_pie_canvas)
        row1_charts_layout.addWidget(sector_allocation_group, 1)
        ...
        self.geo_pie_fig = Figure(figsize=PIE_CHART_FIG_SIZE, dpi=CHART_DPI)
        self.geo_pie_ax = self.geo_pie_fig.add_subplot(111)
        self.geo_pie_canvas = FigureCanvas(self.geo_pie_fig)
        self.geo_pie_canvas.setObjectName("GeoPieCanvas")
        geo_allocation_layout.addWidget(self.geo_pie_canvas)
