    ...
    try:
        ...
        for table in tables:
            try:
                ...
                assert count >= 0, (
                    f"Table '{table}' count query returned invalid result"
                )
        ...
        cursor.execute(
            "SELECT count(*) FROM intraday_ohlcv WHERE close <= 0 OR high < low"
        )
        bad_rows = cursor.fetchone()[0]
        assert bad_rows == 0, (
            f"Found {bad_rows} rows with invalid prices (<=0 or high < low) in intraday_ohlcv"
        )

        # Check for future dates (naive check)
        now_iso = datetime.now().isoformat()
        cursor.execute(
            "SELECT count(*) FROM intraday_ohlcv WHERE timestamp > ?", (now_iso,)
        )
        future_rows = cursor.fetchone()[0]
        if future_rows > 0:
            # Just warn for future timestamps as timezone diffs can cause minor issues
            print(
                f"WARN: Found {future_rows} rows with future timestamps in intraday_ohlcv"
            )
