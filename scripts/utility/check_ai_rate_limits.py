...
def main():
    ...
    for model in FALLBACK_MODELS:
        ...
        if ratelimit_info:
            ...
            print(
                f"  -> Estimated daily reset in: {hours}h {minutes}m (approx. {reset_time_str} local time)"
            )
            print(
                "  -> Note: If this is a minute-based limit, it may reset in < 1 minute."
            )
    ...
    summary = (
        "System Ready"
        if any("AVAILABLE" in s for m, s in results)
