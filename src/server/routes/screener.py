    ...
    try:
        ...
        if universe_type == "holdings":
            summary_data = await _calculate_portfolio_summary_internal(
                data=data, current_user=current_user, show_closed_positions=False
            )
            summary_df = summary_data.get("summary_df")
            if summary_df is not None and not summary_df.empty:
                # Filter out "Total" row and get unique symbols
                manual_symbols = summary_df[
                    ~summary_df.get("is_total", False)
...
def narrative_screener(
    item: Dict[str, str], current_user: User = Depends(get_current_user)
):
...
def trigger_ai_review(
    ...
    try:
        ...
        ratios_data = {
            "Return on Equity (ROE) (%)": fund_data.get("returnOnEquity", 0) * 100
            if fund_data.get("returnOnEquity")
            else None,
            "Gross Profit Margin (%)": fund_data.get("grossMargins", 0) * 100
            if fund_data.get("grossMargins")
            else None,
            "Net Profit Margin (%)": fund_data.get("profitMargins", 0) * 100
            if fund_data.get("profitMargins")
            else None,
            "Debt-to-Equity Ratio": fund_data.get("debtToEquity", 0) / 100
            if fund_data.get("debtToEquity")
            else None,
            "Current Ratio": fund_data.get("currentRatio"),
        }

        review = generate_stock_review(
            symbol, fund_data, ratios_data, force_refresh=force
        )
