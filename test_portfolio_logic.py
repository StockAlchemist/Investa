...
from portfolio_logic import (
    ...
    calculate_irr,
    get_conversion_rate,
    get_historical_fx_rate, # Will need mock data
...
def test_calculate_npv(sample_dates, sample_flows_invest):
    ...
    exp_npv = -1000 + (-500 / (1 + rate)**(days1/365.0)) + (1650 / (1 + rate)**(days2/365.0))
    assert calculate_npv(rate, sample_dates, sample_flows_invest) == pytest.approx(exp_npv)

def test_calculate_npv_zero_rate(sample_dates, sample_flows_invest):
    # With 0 rate, NPV is just the sum of flows
    assert calculate_npv(0.0, sample_dates, sample_flows_invest) == pytest.approx(sum(sample_flows_invest))
...
def test_calculate_irr_standard(sample_dates, sample_flows_invest):
    ...
    assert calculate_npv(irr, sample_dates, sample_flows_invest) == pytest.approx(0.0, abs=1e-5)
...
def test_calculate_irr_invalid_flows(sample_dates, sample_flows_all_neg, sample_flows_all_pos, sample_flows_first_pos):
    ...
    assert pd.isna(calculate_irr(sample_dates, sample_flows_first_pos)) # First flow must be negative
...
def test_get_conversion_rate_direct(sample_current_fx_vs_usd):
    ...
    assert get_conversion_rate('USD', 'EUR', sample_current_fx_vs_usd) == pytest.approx(0.92)
    # USD -> JPY (Expect: JPY per USD)
    assert get_conversion_rate('USD', 'JPY', sample_current_fx_vs_usd) == pytest.approx(145.0)

def test_get_conversion_rate_inverse(sample_current_fx_vs_usd):
    # EUR -> USD (Expect: USD per EUR = 1 / (EUR per USD))
    assert get_conversion_rate('EUR', 'USD', sample_current_fx_vs_usd) == pytest.approx(1 / 0.92)
    # JPY -> USD (Expect: USD per JPY = 1 / (JPY per USD))
    assert get_conversion_rate('JPY', 'USD', sample_current_fx_vs_usd) == pytest.approx(1 / 145.0)

def test_get_conversion_rate_cross(sample_current_fx_vs_usd):
    # EUR -> JPY (Expect: (JPY per USD) / (EUR per USD))
    assert get_conversion_rate('EUR', 'JPY', sample_current_fx_vs_usd) == pytest.approx(145.0 / 0.92)
    # THB -> EUR (Expect: (EUR per USD) / (THB per USD))
    assert get_conversion_rate('THB', 'EUR', sample_current_fx_vs_usd) == pytest.approx(0.92 / 34.5)

def test_get_conversion_rate_missing(sample_current_fx_vs_usd):
    # Missing GBP in dict
    assert get_conversion_rate('USD', 'GBP', sample_current_fx_vs_usd) == 1.0 # Fallback
    assert get_conversion_rate('GBP', 'EUR', sample_current_fx_vs_usd) == 1.0 # Fallback
...
def test_load_clean_transactions(sample_transactions_csv_string, sample_account_map):
    df, _, ignored_indices, ignored_reasons, has_errors, has_warnings = _load_and_clean_transactions(
        StringIO(sample_transactions_csv_string),
        sample_account_map,
        'USD'
    )
    assert df is not None, "DataFrame should not be None after successful load and clean" # Add check for None df
    ...
    assert ignored_reasons.get(10) == "Missing Qty/Price Stock" # Orig idx 10 (BAD_DATE) likely has NaN Qty/Price too
    assert ignored_reasons.get(11) == "Missing Qty/Price Stock" # Orig idx 11 (BAD_PRICE) has NaN Price
    # --- END MODIFICATION ---
...
def test_calculate_cash_balances(sample_transactions_df):
    ...
    assert cash_summary['AccountA']['qty'] == pytest.approx(1000.0 - 200.0) # Qty reflects balance change
    assert cash_summary['AccountA']['commissions'] == pytest.approx(-10.0) # Fees charged
...
def test_process_transactions_holdings_basic(sample_transactions_df):
    ...
    # --- >> END ADD << ---
    holdings, realized, dividends, commissions, ignored_i, ignored_r, has_warnings = _process_transactions_to_holdings(
        stock_tx, 'USD', SHORTABLE_SYMBOLS
    )
    ...
    assert aapl_hold['commissions_local'] == pytest.approx(5.0 + 5.0 + 5.0) # 3 transactions with fees
    assert aapl_hold['total_buy_cost_local'] == pytest.approx(1505.0) # Only the first buy cost
    assert aapl_hold['cumulative_investment_local'] == pytest.approx(-415.0) # Final cumulative investment
