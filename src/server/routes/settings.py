...
def _get_symbol_currency_map_sql(db_path: str) -> Dict[str, str]:
    ...
    try:
        ...
        cursor.execute(
            "SELECT DISTINCT Symbol, \"Local Currency\" FROM transactions WHERE Symbol IS NOT NULL AND Symbol != ''"
        )
...
def get_settings(
    ...
    try:
        ...
        manual_overrides = config_manager.manual_overrides.get(
            "manual_price_overrides", {}
        )
        user_symbol_map = config_manager.manual_overrides.get("user_symbol_map", {})
        user_excluded_symbols = config_manager.manual_overrides.get(
            "user_excluded_symbols", []
        )
        account_currency_map = config_manager.gui_config.get(
        ...
        return {
            ...
            "account_group_order": config_manager.gui_config.get(
                "account_group_order", []
            ),
            "available_currencies": config_manager.gui_config.get(
            ...
            "account_cash_mode_map": config_manager.gui_config.get(
                "account_cash_mode_map", {}
            ),
            "account_closure_dates": config_manager.gui_config.get(
                "account_closure_dates", {}
            ),
            "ibkr_token": config_manager.manual_overrides.get("ibkr_token")
    ...
    try:
        ...
        with config_manager.settings_lock():
            ...
            if settings.manual_price_overrides is not None:
                current_overrides["manual_price_overrides"] = (
                    settings.manual_price_overrides
                )
            ...
            if settings.user_excluded_symbols is not None:
                current_overrides["user_excluded_symbols"] = sorted(
                    list(set(settings.user_excluded_symbols))
                )

            if settings.account_interest_rates is not None:
                current_overrides["account_interest_rates"] = (
                    settings.account_interest_rates
                )

            if settings.interest_free_thresholds is not None:
                current_overrides["interest_free_thresholds"] = (
                    settings.interest_free_thresholds
                )
            ...
            if settings.target_allocation is not None:
                config_manager.gui_config["target_allocation"] = (
            ...
            if settings.account_group_order is not None:
                config_manager.gui_config["account_group_order"] = (
                    settings.account_group_order
                )
                gui_config_changed = True

            if settings.account_currency_map is not None:
                config_manager.gui_config["account_currency_map"] = (
                    settings.account_currency_map
                )
                gui_config_changed = True

            if settings.account_cash_mode_map is not None:
                config_manager.gui_config["account_cash_mode_map"] = (
                    settings.account_cash_mode_map
                )
                gui_config_changed = True

            if settings.account_closure_dates is not None:
                config_manager.gui_config["account_closure_dates"] = (
                    settings.account_closure_dates
                )
                gui_config_changed = True

            if settings.available_currencies is not None:
                config_manager.gui_config["available_currencies"] = (
                    settings.available_currencies
                )
                gui_config_changed = True

            if settings.display_currency is not None:
                config_manager.gui_config["display_currency"] = (
                    settings.display_currency
                )
                gui_config_changed = True

            if settings.selected_accounts is not None:
                config_manager.gui_config["selected_accounts"] = (
            ...
            if config_manager.save_manual_overrides(current_overrides):
                ...
                project_overrides_file = os.path.join(
                    project_root, "manual_overrides.json"
                )
                if os.path.exists(project_overrides_file):
                    try:
                        with open(project_overrides_file, "w", encoding="utf-8") as f:
                            json.dump(
                                current_overrides, f, indent=4, ensure_ascii=False
                            )
                        logging.info(
                            f"Mirrored settings update to {project_overrides_file}"
                        )
                    except Exception as e:
                        logging.warning(
                            f"Failed to mirror settings to project file: {e}"
                        )
                # ---------------------------------------------------------
                ...
                return {
                    "status": "success",
                    "message": "Settings updated and data reloaded",
                }
            else:
                raise HTTPException(
...
def update_manual_override(
    ...
    try:
        ...
        current_overrides = config_manager.manual_overrides.get(
            "manual_price_overrides", {}
        )
        ...
        if not success:
            raise HTTPException(
                status_code=500, detail="Failed to save manual overrides file."
            )
