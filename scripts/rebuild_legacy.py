...
DB = os.path.join(
    os.path.dirname(__file__), "..", "data", "users", "dheematan", "portfolio.db"
)
...
def ins(date, typ, sym, qty, pps, total, comm, acct, note, to_acct=None):
    c.execute(
        """INSERT INTO transactions (Date,Type,Symbol,Quantity,"Price/Share","Total Amount",
...
ins(
    ...
    0,
    "ING Direct",
    "Initial deposit for IDIOX/IDLOX/IDMOX",
)
ins(
    "2004-08-13",
    "deposit",
...
ins(
    ...
    0,
    "ING Direct",
    "Cost basis from 2006 Schedule D",
)
ins(
    "2002-06-29",
    "buy",
...
ins(
    ...
    0,
    "ING Direct",
    "Unknown cost basis - transferred to SB 10/2008",
)
ins(
    "2002-06-29",
    "buy",
...
ins(
    ...
    0,
    "ING Direct",
    "Withdrawal of fund sale proceeds",
)
...
ins(
    "2007-01-04", "withdrawal", "$CASH", 0, 0, -33.13, 0, "Sharebuilder", "CHECK ISSUED"
)
ins("2007-01-09", "withdrawal", "$CASH", 0, 0, -5.94, 0, "Sharebuilder", "CHECK ISSUED")
ins(
    "2007-02-06", "withdrawal", "$CASH", 0, 0, -74.74, 0, "Sharebuilder", "CHECK ISSUED"
)
...
ins(
    ...
    0,
    "Sharebuilder",
    "Statement + Tax Return (combined lots)",
)
ins(
    "2006-02-07",
    "buy",
...
ins(
    ...
    0,
    "Sharebuilder",
    "Amended 2006 D-1 short-term portion",
)
ins(
    "2006-02-24",
    "sell",
    ...
    0,
    "Sharebuilder",
    "Amended 2006 D-1 long-term portion",
)
ins(
    "2006-06-05",
    "sell",
...
ins(
    ...
    0,
    "Sharebuilder",
    "Fractional share liquidation pre-ACATS",
)
ins(
    "2006-12-15",
    "sell",
...
for d, amt in [
    ...
    ins(
        ...
        0,
        "Sharebuilder",
        "Sharebuilder 2006 statement",
    )

# DRV buys (dividend reinvestment fractional shares)
for d, sym, qty in [
    ("2006-04-28", "DIA", 0.1146),
    ("2006-04-28", "SPY", 0.0434),
    ("2006-04-28", "QQQQ", 0.0067),
    ("2006-04-28", "ADRE", 0.1281),
    ("2006-07-31", "DIA", 0.1166),
    ("2006-07-31", "SPY", 0.4054),
    ("2006-07-31", "QQQQ", 0.0757),
    ("2006-07-31", "ADRE", 1.5521),
    ("2006-10-31", "SPY", 0.3944),
    ("2006-10-31", "QQQQ", 0.0603),
...
for sym, qty in [("AAPL", 452), ("VGK", 84), ("SPY", 94), ("QQQQ", 109), ("ADRE", 267)]:
    ins(
        ...
        0,
        "Sharebuilder",
        "ACATS Transfer: Sharebuilder -> TD Ameritrade",
        "TD Ameritrade",
    )
...
ins(
    ...
    0,
    "Penson",
    "Deposit for SPY/VWO/AAPL buys",
)

for buy_d, sell_d, sym, qty, cost, proceeds in penson_2006:
    pps_buy = cost / qty
    ins(
        buy_d, "buy", sym, qty, pps_buy, -cost, 0, "Penson", "Amended 2006 Schedule D-1"
    )
