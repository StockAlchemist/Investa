...
def parse_document_with_ai(file_path: str, mime_type: str) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        for model in VISION_MODELS:
            ...
            try:
                logging.info(
                    f"Vision Parser: Uploading {os.path.basename(file_path)} to {model}..."
                )
                ...
                if response.status_code == 429:
                    logging.warning(
                        f"Vision Parser: Rate limit hit for {model}. Trying next..."
                    )
                    continue
                ...
                if "candidates" in result_data and result_data["candidates"]:
                    content = result_data["candidates"][0]["content"]["parts"][0][
                        "text"
                    ]
                    ...
                    try:
                        ...
                        if start_obj != -1 and (
                            ...
                            if isinstance(data, dict) and "transactions" in data:
                                ...
                                if "statement_info" in data:
                                    logging.info(
                                        f"Vision Parser: Extracted statement info: {data['statement_info']}"
                                    )
                            else:
                        ...
                        logging.info(
                            f"Vision Parser: Successfully extracted {len(transactions)} transactions using {model}."
                        )
                        return transactions
                    except (json.JSONDecodeError, ValueError) as e:
                        logging.warning(
                            f"Vision Parser: Failed to parse JSON from {model}: {e}"
                        )
                        continue
