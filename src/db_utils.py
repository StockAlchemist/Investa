...
def update_transaction_in_db(
    ...
    try:
        ...
        # A transfer is one row carrying "To Account", so there is no other side
        # to update. This used to rewrite Date/Total Amount/Note on every row
        # sharing the edited transfer's Note — unrelated deposits included — and
        # only escaped corrupting them because that write was never committed.
        return True
    except sqlite3.Error as e:
        logging.error(
...
def update_ai_review_in_cache(
    ...
    def do_update(conn, sym, data, inf, univ):
        ...
        try:
            ...
            logging.error(f"Error in do_update for {sym}: {e}")
            conn.rollback()
            return False
...
def update_intrinsic_value_in_cache(
    ...
    def do_iv_update(conn, sym, iv, mos, fy_end, qtr, inf, univ):
        ...
        try:
            ...
            logging.error(f"Error in do_iv_update for {sym}: {e}")
            conn.rollback()
            return False
