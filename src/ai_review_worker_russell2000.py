...
"""
-------------------------------------------------------------------------------
 Name:          ai_review_worker_russell2000.py
 Purpose:       Standalone worker to generate AI stock reviews for Russell 2000 companies.
                Runs independently from the main S&P 500/400 worker.

 Usage:         Run from the 'src' directory or with 'src' in PYTHONPATH.
                python src/ai_review_worker_russell2000.py
...
try:
    ...
    print(
        f"CRITICAL: Failed to import modules. Make sure you are running with 'src' in PYTHONPATH. Error: {e}"
    )
    sys.exit(1)
...
def exit_due_to_quota(target_hour=QUOTA_RESET_HOUR):
    """
    Exits the worker when the AI quota is full.
    """
    ...
    logging.error(
        f"Quota limit reached or excessive failures. Next reset at: {target_time.strftime('%Y-%m-%d %H:%M:%S')}"
    )
...
def check_if_review_exists(
    symbol: str, fund_data: dict = None, universe: str = "russell2000"
) -> bool:
    """
        ...
        if row:
            ...
            if has_review:
                if "error" in summary_text.lower() or len(summary_text) < 20:
                    logging.warning(
                        f"Invalidating cache for {symbol} ({universe}): AI Summary indicates failure or is too short."
                    )
                    return False
            ...
            if has_review and fund_data:
                ...
                if cached_fy_end != live_fy_end or cached_mrq != live_mrq:
                    logging.info(
                        f"Invalidating cache for {symbol} ({universe}): New financial data detected. Cached(FY={cached_fy_end}, Q={cached_mrq}) vs Live(FY={live_fy_end}, Q={live_mrq})"
                    )
                    return False
...
def save_review_to_screener(
    symbol: str, review: dict, fund_data: dict, universe: str = "russell2000"
):
    """
    ...
    try:
        ...
        logging.info(
            f"Saved review to screener cache for {symbol} (universe='{universe}')."
        )
    except Exception as e:
...
def process_stock(
    symbol: str, mdp, fund_data: dict, universe: str = "russell2000"
) -> bool:
    """
    ...
    try:
        ...
        if (
            ...
            try:
                ratios_df = calculate_key_ratios_timeseries(
                    financials_df, balance_sheet_df
                )
        ...
        try:
            review = generate_stock_review(
                symbol, fund_data, ratios, use_search=USE_GROUNDING
            )
        except Exception as e:
        ...
        if "error" in review:
            ...
            if review["error"] == "QUOTA_EXHAUSTED":
        ...
        try:
            ...
            logging.error(
                f"Failed to calculate/update intrinsic value for {symbol}: {iv_e}"
            )
...
def process_ticker_list(mdp, tickers: List[str], universe: str) -> int:
    ...
    """
    logging.info(
        f"Starting processing for universe: {universe} ({len(tickers)} tickers)"
    )
    ...
    for symbol in tickers:
        try:
            ...
            logging.info(
                f"Review needed for {symbol} in {universe}. Starting generation."
            )
            success = process_stock(symbol, mdp, fund_data, universe=universe)

            if success == "QUOTA_EXHAUSTED":
                logging.error("Hard quota exhausted. Exiting immediately.")
                exit_due_to_quota()
            elif success:
                consecutive_failures = 0
                processed_count += 1
                ...
                if consecutive_failures >= MAX_CONSECUTIVE_FAILURES:
                    logging.error(
                        f"Hit max consecutive failures ({MAX_CONSECUTIVE_FAILURES}). Likely Rate Limit or Network Issue."
                    )
                    exit_due_to_quota()
...
def main():
    ...
    logging.info("Starting Russell 2000 AI Review Worker...")
    ...
    while True:
        try:
            ...
            logging.info("Fetching Russell 2000 list...")
            russell_tickers = get_russell2000_tickers()
            total_processed += process_ticker_list(mdp, russell_tickers, "russell2000")

            if total_processed == 0:
                logging.info(
                    "No stocks needed processing (all cached). Sleeping for 1 hour before re-checking..."
                )
                time.sleep(3600)
            else:
                logging.info(
                    f"Cycle complete. Processed {total_processed} stocks. Sleeping for 1 hour..."
                )
                time.sleep(3600)
